				...
				<form name="userForm" class="w-50 ms-2 mt-4">
					<div class="mb-3">
						<label for="username" class="form-label">Username</label> <input
							disabled required ng-model="formState.id" type="text" name="id"
							class="form-control" id="username" />
					</div>
					<div class="mb-3">
						<label for="password" class="form-label">Password</label> <input
							required ng-model="formState.password" type="text"
							class="form-control" id="password" />
					</div>
					<div class="mb-3">
						<label for="fullname" class="form-label">Fullname</label> <input
							required ng-model="formState.fullname" type="text"
							class="form-control" id="fullname" />
					</div>
					<div class="mb-3">
						<label for="email" class="form-label">Email address</label> <input
							required ng-model="formState.email" type="email"
							class="form-control" id="email" />
					</div>
					<div class="d-flex justify-content-start align-items-center">
						<button ng-disabled="isLoading" ng-click="update($event)"
							class="btn btn-primary ms-2">Update</button>
						<button ng-disabled="isLoading" ng-click="delete($event)"
							class="btn btn-danger ms-2">Delete</button>
						<div ng-if="isLoading" class="spinner-border text-primary ms-2"
							role="status"></div>
					</div>
				</form>
				<div ng-show="!!successMessage"
					class="alert alert-success mt-3 w-50 custom-alert-theme"
					role="alert">{{successMessage}}</div>
				<div ng-show="!!errorMessage"
					class="alert alert-danger mt-3 w-50 custom-alert-theme"
			...
			<div class="tab-pane fade show active" id="profile" role="tabpanel"
				...
				<div>
					<jsp:include page="../common/pagination.jsp">
						...
						<jsp:param value="${pageContext.request.contextPath}/admin/users"
							name="baseUrl" />
